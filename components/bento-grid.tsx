...
const features = [
  {
    icon: Palette,
    title: "Web Design",
    description: "Clean, modern interfaces that convert visitors into clients.",
  },
  {
    icon: Zap,
    title: "Development",
    description: "Fast, accessible sites built on modern tech that scale.",
  },
  {
    icon: TrendingUp,
    title: "SEO",
    description: "Rank higher on Google. Get found by the right people.",
  },
  {
    icon: Wrench,
    title: "Maintenance",
    description: "Ongoing support so your site never breaks in production.",
  },
]
...
function FeatureCard({ feature, index }: { feature: (typeof features)[0]; index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Card */}
      <div className="relative bg-[#111111] rounded-2xl p-5 border border-white/10 overflow-hidden h-full">
        ...
        <div className="relative z-10 flex flex-col h-full min-h-[140px]">
          ...
          <motion.div
            className="w-10 h-10 rounded-xl flex items-center justify-center mb-4 relative bg-white/10"
          ...
          >
            <motion.div
              ...
              animate={isHovered ? { opacity: [0.1, 0.2, 0.1], scale: [1, 1.2, 1] } : { opacity: 0, scale: 0.8 }}
              transition={{ duration: 1.5, repeat: Number.POSITIVE_INFINITY }}
            />
            <feature.icon className="w-5 h-5 relative z-10 text-white" />
          </motion.div>
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-white/50 mt-2 font-mono">{feature.description}</p>
          </div>
...
export function BentoGrid() {
  ...
  return (
    <section id="services" className="relative py-16 bg-[#0a0a0a] overflow-hidden">
      ...
      <div ref={ref} className="max-w-5xl mx-auto px-6 relative z-10">
        ...
        >
          <motion.span
            className="inline-block font-mono text-white/60 text-[10px] tracking-[0.3em] uppercase"
          ...
          {/* Animated underline */}
          <motion.div
            className="h-[2px] w-12 bg-white mx-auto mt-3 rounded-full"
            initial={{ scaleX: 0 }}
